<template>
  <div v-show="activeSection === 'wallet' || activeSection?.value === 'wallet'" class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="relative overflow-hidden rounded-3xl border border-indigo-500/30 bg-gradient-to-br from-slate-950 via-indigo-950/80 to-slate-950 p-6 sm:p-8 shadow-2xl shadow-indigo-950/50 backdrop-blur-xl ring-1 ring-indigo-500/20">
      
      <!-- Ambient Glow Effects -->
      <div class="absolute -top-24 -right-24 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl pointer-events-none"></div>
      ...
      <div class="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            <p class="text-xs font-black uppercase tracking-widest text-indigo-300">Tổng Số Dư Khả Dụng</p>
            <h2 class="mt-1 text-4xl sm:text-5xl font-black tracking-tight bg-gradient-to-r from-white via-slate-100 to-indigo-200 bg-clip-text text-transparent">
        ...
        <div class="w-full lg:w-80 rounded-2xl border border-indigo-400/30 bg-gradient-to-tr from-indigo-900/90 via-slate-900/90 to-indigo-950/90 p-5 shadow-2xl shadow-indigo-950/60 space-y-4 flex-shrink-0 backdrop-blur-md">
          <div class="flex justify-between items-start">
            <div>
              <span class="text-[10px] font-black uppercase tracking-widest text-indigo-300">QuickWork Pay</span>
          ...
          <div class="pt-2">
            ...
            <p class="text-xs font-mono font-bold text-indigo-200 tracking-wider">**** **** **** {{ String(wallet?.id || '8888').slice(-4) }}</p>
          </div>

          <div class="flex justify-between items-center pt-2 border-t border-indigo-500/20 text-xs">
            <button
              @click="fetchWallet()"
              class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-500/20 text-indigo-200 hover:bg-indigo-500/30 font-bold transition-all text-[11px]"
            >
      ...
      <!-- Quick Metrics Bar -->
      <div class="mt-8 grid gap-4 grid-cols-1 sm:grid-cols-3 pt-6 border-t border-indigo-500/15">
        <div class="rounded-2xl border border-indigo-500/20 bg-slate-950/60 p-4 flex items-center justify-between">
          <div>
            <p class="text-[11px] font-black uppercase tracking-wider text-indigo-300">Số Dư Hiện Tại</p>
        ...
        <div class="rounded-2xl border border-violet-500/20 bg-slate-950/60 p-4">
          <p class="text-[11px] font-black uppercase tracking-wider text-violet-300">Chuẩn Hóa Escrow</p>
    ...
    <div class="rounded-3xl border border-indigo-500/20 bg-slate-900/90 shadow-xl overflow-hidden backdrop-blur-xl">
      <div class="border-b border-indigo-500/15 px-6 py-5 flex items-center justify-between">
        <div>
      ...
      <div v-else class="divide-y divide-indigo-500/10">
        <div
          v-for="tx in (walletTransactions || [])"
          :key="tx.id"
          class="flex items-center justify-between px-6 py-4 hover:bg-indigo-500/5 transition-colors"
        >
          ...
          <div class="text-right">
            ...
            <p class="mt-0.5 text-[10px] font-black uppercase tracking-wider text-indigo-300">
    ...
    <div v-if="isWithdrawModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div class="bg-slate-900/95 rounded-3xl border border-indigo-500/20 shadow-2xl shadow-slate-950 max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div class="p-6 pb-4 border-b border-indigo-500/15 flex items-center justify-between bg-slate-950/60">
          <div>
        ...
        <form @submit.prevent="submitWithdrawRequest" class="p-6 space-y-4">
          <div>
            <label class="block text-xs font-extrabold text-indigo-300 uppercase tracking-wider mb-1.5">Ngân hàng thụ hưởng</label>
            <select v-model="withdrawForm.bank" required class="block w-full px-3.5 py-2.5 border border-indigo-500/20 rounded-xl text-xs bg-slate-950 text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-400 font-medium">
          ...
          <div>
            <label class="block text-xs font-extrabold text-indigo-300 uppercase tracking-wider mb-1.5">Số tài khoản nhận tiền</label>
            <input
              v-model="withdrawForm.accountNumber"
              type="text"
              required
              class="block w-full px-3.5 py-2.5 border border-indigo-500/20 rounded-xl text-xs bg-slate-950 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 font-mono font-bold"
              placeholder="VD: 19038888888888"
            />
          </div>

          <div>
            <label class="block text-xs font-extrabold text-indigo-300 uppercase tracking-wider mb-1.5">Tên chủ tài khoản (In hoa không dấu)</label>
            <input
              v-model="withdrawForm.accountHolder"
              type="text"
              required
              class="block w-full px-3.5 py-2.5 border border-indigo-500/20 rounded-xl text-xs bg-slate-950 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 font-bold uppercase"
          ...
          <div>
            <div class="flex justify-between items-center mb-1.5">
              <label class="text-xs font-extrabold text-indigo-300 uppercase tracking-wider">Số tiền rút (VNĐ)</label>
            ...
            <div class="relative">
              <input
                ...
                step="10000"
                required
                class="block w-full px-3.5 py-2.5 border border-indigo-500/20 rounded-xl text-xs bg-slate-950 text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-400 font-bold"
                placeholder="Tối thiểu 50.000 VNĐ"
              />
              <button
                type="button"
                @click="withdrawForm.amount = String(wallet?.balance || 0)"
                class="absolute right-2.5 top-2 px-2 py-0.5 rounded-lg bg-indigo-500/20 text-indigo-300 text-[10px] font-black hover:bg-indigo-500/30 transition-all"
              >
                Tất cả
              </button>
            </div>
          ...
          <div class="p-3 rounded-xl bg-slate-950/60 border border-indigo-500/10 text-[11px] text-slate-400 leading-relaxed">
